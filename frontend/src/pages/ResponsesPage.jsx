        ...
        <DashboardLayout>
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12">
                <div className="relative">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        <span className="text-[10px] font-bold font-mono tracking-[0.2em] text-primary-500 uppercase">Data Infrastructure</span>
                        <div className="h-[1px] w-12 bg-primary-500/30"></div>
                    </div>
                    <h1 className="text-5xl font-black tracking-tighter text-white font-display">
                        Intelligence <span className="text-primary-500">Report</span>
                    </h1>
                    <p className="text-slate-500 font-medium mt-2 max-w-md leading-relaxed">
                        Analyze and process incoming telemetry from your deployed form nodes.
                    </p>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-8 items-end">
                    <div className="flex-1 w-full">
                        <div className="flex items-center gap-2 mb-2 ml-1">
                            ...
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono">Source Node Filter</label>
                        </div>
                        <select
                        ...
                        >
                            <option value="">All network nodes active</option>
                            {forms.map(form => (
                                <option key={form.id} value={form.id}>{form.title} (ID: {form.id})</option>
                            ))}
                        </select>
                    </div>
                </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-32 space-y-4">
                    ...
                    <span className="text-[10px] font-bold font-mono text-primary-500 animate-pulse tracking-[0.3em] uppercase">Processing Data Stream...</span>
                </div>
            ) : responses.length > 0 ? (
                        ...
                        <table className="min-w-full divide-y divide-slate-800">
                            <thead className="bg-slate-900/50 backdrop-blur-xl">
                                <tr>
                                    ...
                                    <th className="px-8 py-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] font-mono">Module Source</th>
                                    <th className="px-8 py-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] font-mono">IP Protocol</th>
                                    <th className="px-8 py-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] font-mono">Capture Stamp</th>
                                ...
                                {responses.map((response) => (
                                    <tr key={response.id} className="hover:bg-primary-500/5 transition-all duration-300 group">
                                        <td className="px-8 py-6 whitespace-nowrap">
                                            <span className="text-xs font-mono font-bold text-primary-400">MOD-{String(response.id).padStart(4, '0')}</span>
                                        </td>
                                        <td className="px-8 py-6 whitespace-nowrap">
                                            <div className="text-sm font-bold text-white mb-0.5">{response.form?.title || 'Unknown Source'}</div>
                                            <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest font-mono">eFormX Data Node</div>
                                        </td>
                                        <td className="px-8 py-6 whitespace-nowrap">
                ...
                <div className="glass-card text-center py-32 tech-border group relative overflow-hidden">
                    ...
                    <h3 className="text-3xl font-black text-white mb-3 tracking-tight">No Capture Signals</h3>
                    <p className="text-slate-500 font-medium max-w-sm mx-auto leading-relaxed">
                        Data buffers are currently empty. No telemetry has been received from the active network nodes.
                    </p>
                </div>
            )}
        </DashboardLayout>
